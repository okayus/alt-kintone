...
| 9 | [改善要望・障害報告の受付](impl/phase-9-change-requests.md)（**2本目の業務フロー**） | **済**（2026-08-09） ← 直近 |
...
| 次 | 内容 | 状態 |
|---|---|---|
| **10** | 定義の差分（phase-9 §3 論点E・F・G に整理済み） | **論点整理済み**（着手可能） |

- **フェーズ10（定義の差分）** — 「定義の版と差分」（フェーズ5決定Iで見送った続き。
  §8-2 論点5）はここに合流する。※ phase-9 §3 論点E で「**提案差分は版の仕組み無しで
  作れる**」と分かったので、版を積むのは通知（適用差分）と任意時点比較のためだけになった。
  ⚠ **「フィールドの移動が分かる差分」は §8-2 論点18（定義がフィールドの順序を持たない）が前提**
...
| `@alt/main`（`apps/main`） | **エンドユーザーFE**（React + Vite）。シェル（ナビ・ルータ・**フローごとのAPIクライアント**・時点指定・開発用ユーザー切替・**業務フロー描画 `shell/flow/`**・**起票ボタン**・**未読バッジ**）、**案件一覧のグリッド**（仮想スクロール・フィルタ面・並べ替え・URL 状態＝nuqs・**Enter セル編集** = 型別エディタ・IME ガード・ソフト世代・編集不可の理由表示）、案件詳細（現在地・出口条件チェックリスト＋**未充足の充足のしかた**・遷移・編集フォーム）、**フロー参照画面**（`#/flows/:key` = 遷移グラフ・ステップカード・使うデータ。**APIを叩かず定義だけで描く**）、**要望の一覧・起票フォーム・詳細＋チャット**（`flows/request/`）。**ラベルも含め定義を値として import**。テストは unit + **browser（実 Chromium）** の2層 |

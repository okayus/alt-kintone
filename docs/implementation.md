...
| 7 | [一覧グリッドの書き側](impl/phase-7-list-grid-edit.md)（Enter セル編集） | **済**（2026-08-08） ← 直近 |

フェーズ1〜4（**ブラウザで動作確認できる最小スコープ**）に加えて、フェーズ5で
**業務フローを営業自身が読める**ようになり（`#/flows/sales`）、フェーズ6・7で
**案件一覧が1万件でもスプレッドシートライクに読み書きできる**ようになった。

フェーズ7でできたこと: セルをクリック / 矢印 / Tab で移動し、**Enter で編集 →
Enter で確定**（= `PATCH` 1回 = 有効期間型の1バージョン）。**請求額セルに金額を
入れると自動判定が変わり、未確認件数のセルがその場で動く**（構想の中核が一覧で回る）。
確定してもスクロールと絞り込みは保たれ（ソフト世代）、編集に入れないセルでは
**理由が言葉で出る**（担当者名つき。決定S）。FE テストは unit + **実 Chromium の
browser mode** の2層になり、フォーカスの正直さ・IME ガードは回帰テストで守られる。

次のフェーズは**未定**。候補（着手順は未決定）:
...
`pnpm verify` が通る状態（347 テスト。うち6本は**実 Chromium** で回るコンポーネントテスト）。
...
| `@alt/main`（`apps/main`） | **エンドユーザーFE**（React + Vite）。シェル（ナビ・ルータ・APIクライアント・時点指定・開発用ユーザー切替）、**案件一覧のグリッド**（仮想スクロール・フィルタ面・並べ替え・URL 状態＝nuqs・**Enter セル編集** = 型別エディタ・IME ガード・ソフト世代・編集不可の理由表示）、案件詳細（現在地・出口条件チェックリスト＋**未充足の充足のしかた**・遷移・編集フォーム）、**フロー参照画面**（`#/flows/:key` = 遷移グラフ・ステップカード・使うデータ。**APIを叩かず定義だけで描く**）。**ラベルも含め定義を値として import**。テストは unit + **browser（実 Chromium）** の2層 |

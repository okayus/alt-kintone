...
| 8 | [ロールと参加の認可](impl/phase-8-authz-participation.md)（§8-2 論点12 の解決） | **済**（2026-08-09） ← 直近 |
...
`step.roles` になり、§8-2 論点12 が解けた。後者はフェーズ9（全社員が起票する要望フロー）の前提。
...
**次はフェーズ9（改善要望・障害報告の受付）**。論点整理は済んでいて、前提だった
フェーズ8（論点12 の解決）も片付いた。

| 次 | 内容 | 状態 |
|---|---|---|
| **9** | [改善要望・障害報告の受付](impl/phase-9-change-requests.md)（起票フォーム・チャット） | **論点整理済み**（着手可能） |
| 10 | 定義の差分（フェーズ9 の §3 論点E・F・G） | フェーズ9 の中で整理済み |

- **フェーズ9** — エンドユーザーがスクショに赤ペンで要望を出す運用を、**定義の語彙で
  宣言的に指す**形に置き換える。フェーズ5（フローを画面で読めるようにした）の直接の続き。
  **前提だったフェーズ8 は済**（全ロールが起票する業務が `step.roles` で書けるようになった）
...
`pnpm verify` が通る状態（**361 テスト**。うち6本は**実 Chromium** で回るコンポーネントテスト）。

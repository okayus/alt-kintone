...
| 論点 | 判断 | 帰結 |
|---|---|---|
| **FEのスタック** | **React + Vite**。**ルータ・状態管理・CSSフレームワークは入れない** | 画面が2つの段階で概念を増やさない（`node:http` / `parseArgs` と同じ判断）。ただし React 自体は入れる — **FEは Go 版が来ても捨てない**ので、「TS版は仕様だから素朴に書く」はここには当てはまらない |
...
| 21 | **画面がシェルのレイアウトに触る経路が1つできた**（フェーズ13 で発生）。サイドパネルは viewport に貼り付く（fixed）ので、本文だけでなくヘッダ・ナビ・バナーもそのぶん詰める必要がある。しかしパネルは**詳細画面の中**にマウントされる（決定H。やりとりはレコード1件に紐づくのでシェルに持ち上げる理由が無い）ため、自分より上の `.app` を狭められない。いまは `SidePanel` が `document.documentElement` に `--side-panel-gutter` を書き、`.app` の `padding-right` が読む — **子孫が祖先のレイアウトを変える唯一の穴**。成立しているのは「同時に開くパネルは1つ」だからで、2つ目（一覧の右レール、左サイドのナビ折り畳みなど）が来たら**誰が余白の持ち主か**を決める必要がある。取りうる案: シェルが余白の器を持ち、画面は「ここに出す」とだけ言う（React の portal + context）／ パネルをシェルに持ち上げて決定Hを見直す／ 変数を種類ごとに分けて `.app` が合計する。※ 逆向き（シェルが画面のことを知る）にはしなかった記録として、`App` の `wide` を詳細画面にも広げる案は**捨てた** — `.app-main` の `max-width: 1100px` を残したままで「広い画面では本文が動かない」が成立するため、シェル側に条件を増やす理由が無かった | 低（2つ目が来るまで） |

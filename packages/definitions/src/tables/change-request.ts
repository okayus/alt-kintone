...
export const changeRequest = table(
  'change_request',
  {
    ...
    /**
     * **この要望のためにこう変える予定**、という定義の差分（`BundleDiff`）。
     * `alt diff --request <id>` が書き込む（docs/impl/phase-10-definition-diff.md 決定D）。
     *
     * ⚠ **計算結果を保存する。** 提案差分は適用したら消える（作業ツリーと適用済みが
     * 一致するので同じ計算をしても何も出ない）のに、起票者はあとから
     * 「自分の要望で何が変わったのか」を読みたい。だから中身は**ラベル解決済み**で、
     * 定義がさらに変わっても読める形になっている（§2-1）。
     *
     * ⚠ json は絞り込みにも並べ替えにも使えない。`situation` と同じく読むだけの記録。
     */
    proposal: json('変更案'),
  },
  { label: '改善要望' },
